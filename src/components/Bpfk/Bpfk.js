...
const Bpfk = () => {
  ...
  const getDataInstitusi = async (e) => {
    e.preventDefault();
    try {
      const response = await axiosJWT.get("/apibpfk/bpfk/cari?search=" + cariData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const getAllDataInstitusi = async (e) => {
    try {
      const response = await axiosJWT.get("/apibpfk/bpfk",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  return (
    <div className="gpt3__home section__padding" id="home" style={{ paddingTop: "15px" }}>
      <div className="gpt3__home-content">
        ...
        <h1 className="gradient__text">List Faskes BPFK yang Sudah Registrasi </h1>
