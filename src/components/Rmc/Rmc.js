...
const Rmc = () => {
  ...
  const postValidation = async (param) => {
    try {
      ...
      await axiosJWT.post('/apiregfaskeslain/rmc/validasi', param, customConfig)
      // console.log(param)
  ...
  const getDataInstitusi = async (e) => {
    e.preventDefault();
    try {
      const response = await axiosJWT.get("/apiregfaskeslain/rmc/cari?search=" + cariData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const getAllDataInstitusi = async (e) => {
    try {
      const response = await axiosJWT.get("/apiregfaskeslain/rmc",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  return (
    <div className="gpt3__home section__padding" id="home" style={{ paddingTop: "15px" }}>
      <div className="gpt3__home-content">
        ...
        <h1 className="gradient__text">List Faskes RMC yang Sudah Registrasi </h1>
