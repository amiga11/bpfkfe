...
import style from "./rmc.module.css";
...
const RmcValidated = () => {
  ...
  const searchDataRmc = async (e) => {
    e.preventDefault();
    try {
      const response = await axiosJWT.get("/apiregfaskeslain/rmcvalidated/cari?search=" + cariData,
        {
          headers: {
  ...
  const getDataRmcValidated = async (e) => {
    try {
      const response = await axiosJWT.get("/apiregfaskeslain/rmcvalidated",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  return (
    <div className="gpt3__home section__padding" id="home" style={{ paddingTop: "15px" }}>
      <div className="gpt3__home-content">
        <h1 className="gradient__text">List Faskes RMC yang Sudah di Validasi </h1>
